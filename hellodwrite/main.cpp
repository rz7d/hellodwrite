...
LRESULT CALLBACK WndProc(HWND hWnd, UINT msg, WPARAM wParam, LPARAM lParam) {
	...
	switch (msg) {
	case WM_CREATE: {
		if (PrintError(_T("D2D1CreateFactory"), D2D1CreateFactory(D2D1_FACTORY_TYPE_SINGLE_THREADED, &d2dFactory))) {
			goto error;
		}
		if (PrintError(_T("DWriteCreateFactory"), DWriteCreateFactory(DWRITE_FACTORY_TYPE_SHARED, __uuidof(IDWriteFactory),
